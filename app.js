document.addEventListener("DOMContentLoaded", function () {
    ...
    let currencyButtons1 = document.querySelectorAll(".currency1 button");
    let currencyButtons2 = document.querySelectorAll(".currency2 button");
    ...
    document.getElementById("amount-one").addEventListener("input", function () {
      ...
      document.getElementById("amount-one").addEventListener("keyup", (event) => {
        if (event.target.value.split("")[0] == "." || event.target.value.split("")[0] == "-") {
            ...
            updateConvertedAmount(Number(event.target.value, "amount-two", ".currency2 button.selected", ".currency1 button.selected", ".rate"))

        }else{
          updateConvertedAmount("amount-one", "amount-two", ".currency2 button.selected", ".currency1 button.selected", ".rate");
    ...
    document.getElementById("amount-two").addEventListener("input", function () {
      ...
      document.getElementById("amount-two").addEventListener("keyup", (event) => {
        ...
        if (event.target.value.split("")[0] == "." || event.target.value.split("")[0] == "-") {
            ...
            arr.shift();
            event.target.value = Number(arr.join(""));
            updateConvertedAmount(Number(event.target.value, "amount-one", ".currency1 button.selected", ".currency2 button.selected", ".rate2"))

        }else{
         
          updateConvertedAmount("amount-two", "amount-one", ".currency1 button.selected", ".currency2 button.selected", ".rate2");
  ...
  function updateExchangeRate() {
    ...
    fetch(apiUrl)
      ...
      .then(data => {
        ...
        updateConvertedAmount("amount-one", "amount-two", ".currency1 button.selected", ".currency2 button.selected", ".rate2");
        updateConvertedAmount("amount-one", "amount-two", ".currency2 button.selected", ".currency1 button.selected", ".rate");
      })
  ...
  function updateExchangeRate2() {
    ...
    fetch(apiUrl)
      ...
      .then(data => {
        ...
        rateElement2.textContent = `1 ${toCurrency} = ${exchangeRate} ${fromCurrency}`;
        rateElement.textContent = `1 ${fromCurrency} = ${exchangeRate} ${toCurrency}`;
        updateConvertedAmount2("amount-two", "amount-one", ".currency1 button.selected", ".currency2 button.selected", ".rate");
        updateConvertedAmount2("amount-two", "amount-one", ".currency2 button.selected", ".currency1 button.selected", ".rate2");
      })
      .catch(error => console.error("Error fetching exchange rates:", error));
  }
